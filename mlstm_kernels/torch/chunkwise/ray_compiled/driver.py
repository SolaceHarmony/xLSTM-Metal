...
def _ensure_ray():
    ...
    if not ray.is_initialized():
        # Default to local_mode to keep execution in-process (no tensor copies, GPU-only)
...
def mlstm_chunkwise__ray_compiled_steps(
    ...
    **kwargs,
    ...
    # Dispatch per-band in chunk tiles
    pending = []
    for hs, he, actor in actors:
        ...
        while s < S:
            ...
            # Next chunk starts with None; we’ll stitch states from results when ready
            band_states[(hs, he)] = None
            s = e
