...
def soft_cap(values: torch.Tensor, cap_value: float | torch.Tensor | None) -> torch.Tensor:
    """Soft-cap values with strict Metal path on MPS.

    Requires MPS and uses the Metal kernel. No CPU fallback.
    """
    if cap_value is None:
        return values
    if torch.backends.mps.is_available():
        if not values.is_mps:
            raise RuntimeError("soft_cap requires MPS tensor on MPS-enabled system")
        if isinstance(cap_value, torch.Tensor):
            cap = float(cap_value.item())
        else:
            cap = float(cap_value)
        from mlstm_kernels.torch.metal.softcap import metal_soft_cap as _metal_soft_cap
        return _metal_soft_cap(values, cap)
    raise RuntimeError("MPS not available; GPU-only configuration requires Metal")
