...
def load_from_pretrained(
    ...
    # Apple-defaults: on MPS, prefer compiled graph backends by default
    if torch.backends.mps.is_available():
        if chunkwise_kernel_name is None:
            chunkwise_kernel_name = "chunkwise--native_compiled_autograd"
        if sequence_kernel_name is None:
            sequence_kernel_name = "native_sequence__metal"
        if step_kernel_name is None:
            step_kernel_name = "metal"
