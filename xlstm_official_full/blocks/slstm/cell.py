...
class sLSTMCellConfig:
    ...
    backend: Literal["vanilla", "cuda", "native_compiled"] = "cuda"
    ...
    def __post_init__(self):
        # On Apple/MPS, prefer the compiled native backend by default
        try:
            if torch.backends.mps.is_available() and self.backend == "cuda":
                self.backend = "native_compiled"
        except Exception:
            pass
        if self.num_heads <= 0:
            self.num_heads = 1
...
# Compiled (torch.compile) backend that mirrors the vanilla implementation
def _slstm_forward_compiled_eager(x, states, R, b, pointwise, constants):
    # Strict device check: compiled path must run on GPU
    if x.device.type == 'cpu':
        raise RuntimeError("sLSTM compiled backend requires GPU (MPS/CUDA); CPU not allowed.")
    return slstm_forward(x, states, R, b, pointwise, constants=constants)

def _slstm_forward_step_compiled_eager(x, states, R, b, pointwise, constants):
    # Strict device check: compiled path must run on GPU
    if x.device.type == 'cpu':
        raise RuntimeError("sLSTM compiled backend requires GPU (MPS/CUDA); CPU not allowed.")
    return slstm_forward_step(x, states, R, b, pointwise, constants=constants)

try:
    _slstm_forward_compiled = torch.compile(
        _slstm_forward_compiled_eager, backend="inductor", mode="reduce-overhead"
    )
    _slstm_forward_step_compiled = torch.compile(
        _slstm_forward_step_compiled_eager, backend="inductor", mode="reduce-overhead"
    )
except Exception as e:
    raise RuntimeError(
        f"torch.compile failed for sLSTM compiled backend: {e}. No fallback allowed."
    )


class sLSTMCell_compiled(sLSTMCellBase):
    config_class = sLSTMCellConfig
    ...
    def _impl(
        ...
        return _slstm_forward_compiled(
            input,
            state,
            self._recurrent_kernel,
            self._bias,
    ...
    def _impl_step(
        ...
        return _slstm_forward_step_compiled(
            input,
            state,
            self._recurrent_kernel,
            self._bias,
...
class sLSTMCell(sLSTMCellBase):
    ...
    def __new__(cls, config: sLSTMCellConfig, skip_backend_init: bool = False):
        ...
        elif config.backend == "native_compiled":
            return sLSTMCell_compiled(config)
        else:
            raise RuntimeError(
